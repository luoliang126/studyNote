...
注意：如果是根目录会在app.module.ts中自动引入该组件，并注册。如果是在具体某一文件夹下，则会在最近的module模块中引入该组件，并注册。
在项目中直接使用<app-heroes></app-heroes>
                
2、创建一个路由模块
注意：如果在ng new my-app的时候选择了路由模块的话，会自动生成app-routing.modules.ts，就不需要再单独创建路由模块了，如果最开始创建项目时没选择router，那么后面需要的时候就要单独创建。
