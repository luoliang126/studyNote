...
6. ##### 设计模式 https://www.cnblogs.com/imwtr/p/9451129.html
   ...
   发布订阅模式的特点：
   3.1、买家（订阅者）只要声明对消息的一次订阅，就可以在未来的某个时候接收来自售货员（发布者）的消息，不用一直轮询消息的变化。
...
14. ##### 微前端
    ...
    ```js
    什么叫微前端？
    为了解决庞大的一整块后端服务带来的变更与扩展方面的限制，出现了后端微服务架构。然而，越来越重的前端工程也面临同样的问题，自然地想到了将微服务思想用到前端，于是有了「微前端（micro-frontends）」的概念。即：一种由独立交付的多个前端应用组成整体的架构风格。具体的，将前端应用分解成一些更小、更简单的能够独立开发、测试、部署的小块，但在用户看来仍然是内聚的单个产品应用。
    微前端应该具备以下特点：
    1、技术栈无关：主框架不限制接入应用的技术栈，子应用具备完全自主权（技术选型灵活）
    2、独立开发、独立部署：子应用仓库独立，前后端可独立开发，部署完成后主框架自动完成同步更新，单个模块发生错误,不影响全局。
    3、独立运行时：每个子应用之间状态隔离，运行时状态不共享
    4、扩展: 每一个服务可以独立横向扩展以满足业务伸缩性，与资源的不必要消耗。
    ...
    注意点：
    1.1、iframe高度问题，通过计算内部高度而定，或者匹配当前窗口的宽高。
    1.2、传递参数，需要等到iframe onload之后再进行
