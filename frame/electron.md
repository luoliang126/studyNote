...
8. ##### electron与vue的混合开发
   ...
   第二种，使用electron-vue
   描述：有时候我们的前端项目（基于vue）已经完成，但是客户呢，他不希望我们将项目放到互联网上，他想打包成一个桌面应用程序的方式，这时候electron-vue就充当一个打包壳的功能，然后里面的内容还是vue的SPA应用。具体实施步骤，首先将我们的vue项目打包好生成一个dist目录，然后将dist目录下的所有依赖文件，全部拷贝到electron的根目录下，再修改配置，使用electron再打包外壳，生成一个可以直接运行的.exe文件。
   踩坑记录：
   1、配置vue.config.js
   	publicPath:'./'
